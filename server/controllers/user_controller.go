...
func Login(c *gin.Context) {
	...
	if err := db.Preload("Posts", func(db *gorm.DB) *gorm.DB {
		...
		return db.Omit("User")
	}).Preload("Posts.Comments").Where("username = ?", loginData.Username).First(&user).Error; err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid credentials"})
		return
	}

	// Preload followers without omitting any field
	if err := db.Preload("Followers").First(&user).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error loading followers"})
		return
	}

	// Preload following without omitting any field
	if err := db.Preload("Following").First(&user).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error loading following"})
		return
	}
